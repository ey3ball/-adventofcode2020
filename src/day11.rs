...
impl Floor {
    fn new(input: &str) -> Floor {
        ...
        input.lines().enumerate().for_each(|(y, line)| {
            line.chars().enumerate().for_each(|(x, c)| {
                plan.insert((x as isize, y as isize), c);
                ()
            })
        });
        Floor {
    ...
    fn seats<'a>(&'a self, point: Pt) -> impl Iterator<Item = char> + 'a {
        ...
        patterns.filter(|pt| *pt != (0, 0)).map(move |direction| {
            let mut seen: char = '.';
...
fn part1(floor: &Floor) -> usize {
    ...
    loop {
        ...
        new_floor.plan = prev_floor
            .plan
            .iter()
            .map(|(&k, &v)| {
                if v == 'L' && prev_floor.neighours_values(k).all(|x| x == 'L' || x == '.') {
                    (k, '#')
                } else if v == '#'
                    && prev_floor.neighours_values(k).filter(|&x| x == '#').count() >= 4
                {
                    (k, 'L')
                } else {
                    (k, v)
...
fn part2(floor: &Floor) -> usize {
    ...
    loop {
        ...
        new_floor.plan = prev_floor
            ...
            .iter()
            .map(|(&k, &v)| {
                if v == 'L' && prev_floor.seats(k).all(|x| x == 'L' || x == '.') {
                    (k, '#')
                } else if v == '#' && prev_floor.seats(k).filter(|x| *x == '#').count() >= 5 {
                    (k, 'L')
                } else {
                    (k, v)
                }

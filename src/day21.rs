...
pub fn parse(input: &str) -> Parsed {
    input
        ...
        .map(|l| {
            ...
            let ingredients = l.split(" (").next().unwrap().split(" ").collect();
...
fn part1(input: &str) -> usize {
    ...
    parsed.iter().for_each(|(ingredients, allergens)| {
    ...
    let maybe: HashSet<&str> = contained
        .iter()
    ...
    parsed
        .iter()
        .flat_map(|(is, _)| is.iter().filter(|i| no_allergens.contains(*i)))
        .count()
}
...
fn part2(input: &str) -> usize {
    ...
    let mut dangerous: Vec<(&str, &str)> = contained
        .iter()
